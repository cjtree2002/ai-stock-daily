...
- 摘要中可能包含用户私人的 Notion 研究笔记：它们是重要决策输入，但 commentary 和 journal 都会公开发布——【绝不能在其中引用、复述或提及 Notion 笔记的具体内容、标题或数据】，由笔记启发的观点必须表述为你自己的判断。

## 输出
先完成研究，然后在回复的最后给出三个清晰的部分（纯文字即可，不需要 JSON）：
【晨评】3-5句中文，可分段。结构=目标日市场判断→对组合的影响→今日操作概述或不动的理由。数字只用摘要中的。
【交易】每笔一行：买入/卖出 代码 股数 理由（写明依据）。没有交易就写"无"。卖出不得超过持有数，买入以现金支付，不要写价格——成交价由系统按盘后价执行。
【决策日志】完整覆盖更新后的决策日志（150行内）：①每只持仓的当前论点 ②观察清单（等待的信号/价位/条件）③今日备忘。此文件会公开。"""

SUBMIT_TOOL = {
    "name": "submit_decision",
    "description": "提交组合经理的最终决策（晨评、交易清单、决策日志）",
    "input_schema": {
        "type": "object",
        "properties": {
            "commentary": {"type": "string", "description": "晨评正文，可用\\n分段"},
            "trades": {
                "type": "array",
                "items": {
                    "type": "object",
                    "properties": {
                        "action": {"type": "string", "enum": ["买入", "卖出"]},
                        "ticker": {"type": "string"},
                        "shares": {"type": "integer"},
                        "reason": {"type": "string"},
                    },
            ...
            "journal": {"type": "string", "description": "完整覆盖更新后的决策日志 markdown"},
        },
        "required": ["commentary", "trades", "journal"],
    },
}


def _fetch_notion_notes() -> str:
    """Pull the user's most recently edited Notion notes (盘前/盘后总结、会议
    纪要 etc.) via the official integration API. Fail-soft: any error returns
    empty and the run continues without Notion."""
    ...
    try:
        ...
        for p in pages[:6]:
            ...
            rb = requests.get(
                f"https://api.notion.com/v1/blocks/{p['id']}/children?page_size=80",
                headers=H, timeout=25)
            texts = []
            for b in rb.json().get("results", []):
                obj = b.get(b.get("type")) or {}
                s = "".join(t.get("plain_text", "") for t in (obj.get("rich_text") or []))
                if s.strip():
                    texts.append(s)
            content = "\n".join(texts)[:2500]
            if content.strip():
                out.append(f"### {title or '(无标题)'}\n{content}")
        if out:
            print(f"  Notion: 读取 {len(out)} 篇最近笔记")
        return "\n\n".join(out)
    except Exception as e:
        print(f"  [WARN] Notion 读取失败（继续，不影响运行）: {e}", file=sys.stderr)
        return ""


def _fmt_digest(target, pf, prices, indices, news, journal, total, notion=""):
    lines = []
    ...
    if notion.strip():
        lines.append("\n## 用户本人的 Notion 研究笔记（最近编辑的几篇，权重很高）")
        lines.append(notion)
        lines.append("（注意：笔记观点与市场信息冲突时不要盲从任何一方，在晨评中说明取舍。）")

    lines.append("\n## 你的决策日志（上次运行留下的）")
    lines.append(journal if journal.strip() else "（空——今天是日志的第一天）")

    lines.append("\n请先用网络搜索补充了解 T 日市场驱动与持仓个股动态（最多6次），然后按系统要求给出【晨评】【交易】【决策日志】三部分结论。")
    return "\n".join(lines)


def _with_retry(fn, what, attempts=3):
    """Retry transient API failures (overloaded/rate-limit/network)."""
    ...
    for i in range(attempts):
        try:
            ...
            if i == attempts - 1:
                raise
            print(f"  [retry {i+1}/{attempts-1}] {what}: {e}", file=sys.stderr)
            _time.sleep(15 * (i + 1))


def _call_claude(digest):
    """Phase 1: free-form research & analysis (web search allowed)."""
    import anthropic
    ...
    resp = _with_retry(_go, "research call")
    text = "".join(b.text for b in resp.content if getattr(b, "type", "") == "text")
    usage = getattr(resp, "usage", None)
    if usage:
        print(f"  API usage(research): in={usage.input_tokens} out={usage.output_tokens}")
    if not text.strip():
        raise ValueError("research call returned empty text")
    return text


def _structure_decision(analysis_text):
    """Phase 2: FORCED tool call — the API validates the JSON structure, so a
    malformed free-text JSON can never break us again."""
    import anthropic
    client = anthropic.Anthropic()
    ...
    def _go():
        return client.messages.create(
            model=MODEL,
            max_tokens=6000,
            system="你是数据整理助手。把组合经理的分析结论原样提取为结构化决策并调用 submit_decision 提交。"
                   "晨评、交易、决策日志的内容必须完全取材于结论文本，不要新增或修改观点；"
                   "结论说'无交易'时 trades 提交空数组。",
            messages=[{"role": "user", "content": f"组合经理的分析结论：\n\n{analysis_text}"}],
            tools=[SUBMIT_TOOL],
            tool_choice={"type": "tool", "name": "submit_decision"},
        )

    resp = _with_retry(_go, "structuring call")
    usage = getattr(resp, "usage", None)
    if usage:
        print(f"  API usage(structure): in={usage.input_tokens} out={usage.output_tokens}")
    for b in resp.content:
        if getattr(b, "type", "") == "tool_use" and b.name == "submit_decision":
            return b.input
    raise ValueError("no submit_decision tool_use in response")
...
def main():
    ...
    total = pf["cash"] + sum(h["shares"] * (prices.get(h["ticker"]) or {}).get("price", h["avg_cost"])
                             for h in pf["holdings"])
    digest = _fmt_digest(target, pf, prices, indices, news, journal, total, notion)
    print(f"  digest: {len(digest)} chars")

    text = _call_claude(digest)
    decision = _structure_decision(text)
